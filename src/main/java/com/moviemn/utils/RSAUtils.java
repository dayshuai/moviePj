...
/**
 * RSA算法加密/解密工具类。
 * 公钥和私钥保存在__RSA_PAIR.txt文件中，如果此文件不存在就会生成公钥和私钥保存在__RSA_PAIR.txt文件中并缓存
 * __RSA_PAIR.txt文件存在时公钥和私钥是不变的 RSA速度 : 由于进行的都是大数计算，使得RSA最快的情况也比DES慢上100倍，无论
 * 是软件还是硬件实现。 速度一直是RSA的缺陷。一般来说只用于少量数据 加密。
 * 当你需要更改公钥和私钥时，请删除__RSA_PAIR.txt,以及修改generateKeyPair()方法中的 new
 * SecureRandom()构造数字生成器
 */
public class RSAUtils {
	...
	private static synchronized KeyPair generateKeyPair() {
		try {
			// KEY_SIZE表示KEY_SIZE,new
			// SecureRandom()表示构造随机数字生成器,当这个生成器不变时，所形成的公钥和私钥是不变的。
			// keyPairGen.initialize(KEY_SIZE, new
			// SecureRandom(DateFormatUtils.format(new
			// Date(),"yyyyMMdd").getBytes()));
			keyPairGen.initialize(KEY_SIZE,
					new SecureRandom("r0o5wyt4s70g".getBytes()));
	...
	public static PublicKeyMap getPublicKeyMap() {
		...
		publicKeyMap.setModulus(new String(Hex.encodeHex(rsaPublicKey
				.getModulus().toByteArray())));// 系数
		publicKeyMap.setExponent(new String(Hex.encodeHex(rsaPublicKey
				.getPublicExponent().toByteArray())));// 指数
		publicKeyMap.setToken(String.valueOf(Calendar.getInstance().getTimeInMillis()));
		return publicKeyMap;
	}
}
