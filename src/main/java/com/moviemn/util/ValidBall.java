...
public class ValidBall {
     ...
     * @param red 传入红球
     * @param blue 传入蓝球
     * @param officialRed 开奖红球
     * @param officialBlue 开奖蓝球
     * @return
     */
    // 中奖,传入号码，匹配是否中奖，如果中奖返回中的几等奖，没总返回-1
	public int isMiddle(int[] red,int blue, int [] officialRed, int officialBlue) {
        // 输出投注号码
        ...
        if (blue == officialBlue) {
            middle += 1;// 蓝色球中
        }
        for (int i = 0; i < red.length; i++) {
            if (red[i] == officialRed[i]) {
                middle += 1;// 红色球中
            }
        }
        if (middle != 0) {
